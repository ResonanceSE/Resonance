...
def get_product_filters(request):
    ...
    connections = Product.objects.values('connections').annotate(
        count=Count('connections')
    ).order_by('connections')
    
    # Get price ranges (calculated on the fly)
    price_min = Product.objects.order_by('price').first().price if Product.objects.exists() else 0
    price_max = Product.objects.order_by('-price').first().price if Product.objects.exists() else 1000
    ...
    if Product.objects.exists():
        # Define price ranges 
        if price_min < 100:
            price_ranges.append({"name": "Under $100", "min": 0, "max": 99.99, "count": Product.objects.filter(price__lt=100).count()})
        
        if price_min < 300 and price_max > 100:
            price_ranges.append({"name": "$100 - $300", "min": 100, "max": 299.99, "count": Product.objects.filter(price__gte=100, price__lt=300).count()})
        
        if price_min < 500 and price_max > 300:
            price_ranges.append({"name": "$300 - $500", "min": 300, "max": 499.99, "count": Product.objects.filter(price__gte=300, price__lt=500).count()})
        
        if price_max > 500:
            price_ranges.append({"name": "Over $500", "min": 500, "max": None, "count": Product.objects.filter(price__gte=500).count()})
    else:
